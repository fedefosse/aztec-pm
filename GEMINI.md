...
- **`execution/`** — scripts Python deterministas: `db.py` (esquema/acceso a SQLite, incluida la persistencia del diagnóstico), `import_dataset.py` (carga del dataset semilla + cálculo inicial de prioridad/salud), `risk_engine.py` (funciones puras de riesgo/prioridad/analytics, sin efectos secundarios) y `webapp.py` (rutas Flask: dashboard ejecutivo `/` (carga principal), vista operativa `/proyectos`, equipo `/equipo`, CRUD). `static/live-filters.js` es el único JavaScript del proyecto. `webapp.filter_link`/`apply_filters`/`faceted_summary` son el único mecanismo de filtrado — cualquier vista nueva con filtros debe reutilizarlos.
