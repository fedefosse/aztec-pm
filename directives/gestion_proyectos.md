...
1. **¿Dónde está el cuello de botella?** Ranking de proyectos por Índice de Tensión + salud del portafolio (distribución Bloqueado/En riesgo/Sano).
...
Todo en la página es navegación real: cada tarjeta KPI, gajo del donut y fila de ranking lleva a los proyectos/tareas que la componen en `/` o en el detalle del proyecto — nunca es un número sin forma de auditarlo.

## Interactividad (`static/live-filters.js`)

Los filtros de `/` y `/dashboard` aplican al instante (sin recargar la página) vía un único script vanilla, sin frameworks ni build step: intercepta el `submit` del formulario de filtros y los `click` de los links marcados `data-live-link`, hace `fetch()` de la misma URL con la nueva query string, y reemplaza solo los bloques del DOM marcados por id (`data-live-target="id1,id2,..."`) con lo que el servidor devolvió. El servidor sigue siendo la única fuente de verdad — esto solo evita el reload completo. Si JS falla o está desactivado, el formulario se degrada a un GET normal (recarga completa), que sigue funcionando exactamente igual.
